"""Enhanced work completion dialog with statistics and custom extend time - Fixed version."""
...
class WorkCompletionDialog:
    ...
    def __init__(self, stats: Dict, extend_minutes: int = 5):
        ...
        self.controller = None  # Will hold the delegate
    ...
    def show(self, callback: Optional[Callable[[int, Optional[int]], None]] = None):
        ...
        # Create a controller to handle button actions
        self.controller = DialogController.alloc().init()
        ...
        # Save reference in controller so it can access it
        self.controller.extend_field = self.extend_field
        ...
        primary_btn.setTarget_(self.controller)
        ...
        extend_btn.setTarget_(self.controller)
        ...
        skip_btn.setTarget_(self.controller)
...
class DialogController(AppKit.NSObject):
    """Controller to handle button actions for the dialog."""
    ...
    def extendAction_(self, sender):
        ...
        if self.dialog:
            ...
            try:
                self.dialog.custom_extend_minutes = float(self.extend_field.stringValue())
                if self.dialog.custom_extend_minutes <= 0:
                    self.dialog.custom_extend_minutes = self.dialog.extend_minutes
    ...
    def windowWillClose_(self, notification):
        ...
        if self.dialog and self.dialog.callback:
            self.dialog.callback(None, None)